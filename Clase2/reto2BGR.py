...
def main():
    ...
    # fn.show_image(imageBgr, title="Image BGR")

    while True:
        vals = fn.get_trackbar_values("Binary Image", type=1)
        lowLim = vals[:3]
        upLim = vals[3:]
        binPic = fn.binary(imageBgr, method=2, rgbMin=lowLim, rgbMax=upLim)
        ...
        if val[0] == 0:
            test = fn.replace_color(imageBgr, old_color_min=old_color_min, old_color_max=old_color_max, new_color=(0, 0, 255), color_space="BGR")  # Red in BGR
        elif val[0] == 1:
            test = fn.replace_color(imageBgr, old_color_min=old_color_min, old_color_max=old_color_max, new_color=(0, 255, 0), color_space="BGR")  # Green in BGR
        else:
            test = fn.replace_color(imageBgr, old_color_min=old_color_min, old_color_max=old_color_max, new_color=(255, 0, 0), color_space="BGR")  # Blue in BGR
