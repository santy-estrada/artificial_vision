...
df = pd.read_excel(r'C:\Personal\Samuel\UNIVERSIDAD\Decimo_semestre\VIsion_artificial\Proyecto_deteccion_placas\patrones\caractNums_placas.xlsx', header=None, engine='openpyxl')
y = df.iloc[:, 0]
x = df.iloc[:, 1:]
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.1, random_state=42)

# Entrenamiento del Modelo MLP
model_mlp = MLPClassifier(hidden_layer_sizes=(30,40,20), max_iter=2000, activation='relu', solver='adam', random_state=42)
...
nombre_archivo = 'model_num_placas.joblib'
